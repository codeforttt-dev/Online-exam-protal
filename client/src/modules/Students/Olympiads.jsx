...
const Olympiads = () => {
  const [activeCategory, setActiveCategory] = useState('all'); // Now useState is defined
  ...
  const filteredOlympiads = olympiads.filter(olympiad => {
    ...
    if (activeCategory === 'Active') return olympiad.status === 'Active';
    if (activeCategory === 'Upcoming') return olympiad.status === 'Upcoming';
    if (activeCategory === 'Completed') return olympiad.status === 'Completed';
    return true;
  });
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage your registered olympiads and track progress</p>
        </div>
        <button 
          onClick={handleRegister}
          className="bg-primary hover:bg-primary-dark text-white font-semibold py-2.5 px-5 rounded-lg transition-colors duration-300 flex items-center"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-4 shadow-sm">
          <div className="text-center">
            <div className="text-2xl font-bold text-primary">{olympiads.length}</div>
      ...
      {filteredOlympiads.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No olympiads found</h3>
          <p className="text-gray-600 mb-6">Try a different category or register for a new olympiad</p>
          <button
            onClick={handleRegister}
            className="bg-primary hover:bg-primary-dark text-white font-semibold py-2.5 px-5 rounded-lg transition-colors duration-300"
          >
            Register Now
          </button>
        </div>
